use std::{collections::{HashMap, HashSet}, cell::RefCell, fmt::Display};

use petgraph::{stable_graph::{StableDiGraph, NodeIndex, DefaultIx}, algo::toposort, Direction, visit::IntoNodeIdentifiers};

type NodePositions = HashMap<usize, (isize, isize)>;
...
impl<T: Default> GraphLayout<T> {
    ...
    /// of the graph.
    pub fn create_layouts(edges: &[(u32, u32)], node_size: isize, global_tasks_in_first_row: bool) -> Vec<(NodePositions, usize, usize)> {
